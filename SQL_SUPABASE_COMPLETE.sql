...
(uuid_generate_v4(), 'order_confirmation_template', '{"value": "Your order {order_id} has been confirmed!"}', 'Template for order confirmation messages'),
(uuid_generate_v4(), 'payment_received_template', '{"value": "We have received your payment for order {order_id}."}', 'Template for payment received messages'),
(uuid_generate_v4(), 'order_shipped_template', '{"value": "Your order {order_id} has been shipped!"}', 'Template for order shipped messages'),
(uuid_generate_v4(), 'order_delivered_template', '{"value": "Your order {order_id} has been delivered!"}', 'Template for order delivered messages');
...
BEGIN
    IF TG_OP = 'SELECT' THEN
        UPDATE "Products" SET "view_count" = "view_count" + 1 WHERE "id" = NEW."id";
        RETURN NEW;
    END IF;
    RETURN NULL;
END;
...
-- Create trigger for updating product view count
CREATE TRIGGER product_view_count
AFTER SELECT ON "Products"
FOR EACH ROW EXECUTE FUNCTION increment_product_view_count();

-- Create function to check and assign badges
CREATE OR REPLACE FUNCTION check_and_assign_badges()
RETURNS TRIGGER AS $$
DECLARE
    ...
    avg_rating DECIMAL;
BEGIN
    -- Check if the trigger was called by a new review
    IF TG_OP = 'INSERT' AND NEW."rating" IS NOT NULL THEN
        -- Check seller badges if review is for a seller or product
        IF NEW."seller_id" IS NOT NULL OR NEW."product_id" IS NOT NULL THEN
            IF NEW."seller_id" IS NOT NULL THEN
                seller_record := SELECT * FROM "Sellers" WHERE "id" = NEW."seller_id";
            ELSE
                seller_record := SELECT s.* FROM "Sellers" s JOIN "Products" p ON s."id" = p."seller_id" WHERE p."id" = NEW."product_id";
            END IF;
            
            IF seller_record IS NOT NULL THEN
                -- Get review count and average rating for the seller
                SELECT COUNT(*), AVG(r."rating") INTO review_count, avg_rating
                FROM "Reviews" r
                WHERE r."seller_id" = seller_record."id" OR r."product_id" IN (
                    SELECT p."id" FROM "Products" p WHERE p."seller_id" = seller_record."id"
                );
                
                -- Check and assign Top Rated badge
                IF avg_rating >= 4.5 AND review_count >= 10 THEN
                    INSERT INTO "UserBadges" ("user_id", "badge_id", "earned_at")
                    SELECT seller_record."user_id", b."id", NOW()
                    FROM "Badges" b
                    WHERE b."type" = 'top_rated'
                    AND NOT EXISTS (
                        SELECT 1 FROM "UserBadges" ub 
                        WHERE ub."user_id" = seller_record."user_id" 
                        AND ub."badge_id" = b."id"
                    );
                END IF;
                
                -- Check and assign Quality Products badge
                IF avg_rating >= 4.0 AND review_count >= 5 THEN
                    INSERT INTO "UserBadges" ("user_id", "badge_id", "earned_at")
                    SELECT seller_record."user_id", b."id", NOW()
                    FROM "Badges" b
                    WHERE b."type" = 'quality_products'
                    AND NOT EXISTS (
                        SELECT 1 FROM "UserBadges" ub 
                        WHERE ub."user_id" = seller_record."user_id" 
                        AND ub."badge_id" = b."id"
        ...
        -- Check dog walker badges if review is for a walker or service
        IF NEW."dog_walker_id" IS NOT NULL OR NEW."service_id" IS NOT NULL THEN
            IF NEW."dog_walker_id" IS NOT NULL THEN
                walker_record := SELECT * FROM "DogWalkers" WHERE "id" = NEW."dog_walker_id";
            ELSE
                walker_record := SELECT w.* FROM "DogWalkers" w JOIN "Services" s ON w."id" = s."dog_walker_id" WHERE s."id" = NEW."service_id";
            END IF;
            
            IF walker_record IS NOT NULL THEN
                -- Get review count and average rating for the walker
                SELECT COUNT(*), AVG(r."rating") INTO review_count, avg_rating
                FROM "Reviews" r
                WHERE r."dog_walker_id" = walker_record."id" OR r."service_id" IN (
                    SELECT s."id" FROM "Services" s WHERE s."dog_walker_id" = walker_record."id"
                );
                
                -- Check and assign Trusted Walker badge
                IF avg_rating >= 4.5 AND review_count >= 5 THEN
                    INSERT INTO "UserBadges" ("user_id", "badge_id", "earned_at")
                    SELECT walker_record."user_id", b."id", NOW()
                    FROM "Badges" b
                    WHERE b."type" = 'trusted_walker'
                    AND NOT EXISTS (
                        SELECT 1 FROM "UserBadges" ub 
                        WHERE ub."user_id" = walker_record."user_id" 
                        AND ub."badge_id" = b."id"
                    );
                END IF;
            END IF;
...
-- Create trigger for checking and assigning badges
CREATE TRIGGER review_badge_check
AFTER INSERT OR UPDATE ON "Reviews"
FOR EACH ROW EXECUTE FUNCTION check_and_assign_badges();
...
-- Create trigger for updating order status based on payment
CREATE TRIGGER payment_status_change
AFTER UPDATE ON "Payments"
FOR EACH ROW EXECUTE FUNCTION update_order_status_on_payment();

-- Create function to log actions in audit log
CREATE OR REPLACE FUNCTION log_audit_changes()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'UPDATE' THEN
        INSERT INTO "AuditLogs" ("user_id", "action", "entity_type", "entity_id", "old_values", "new_values")
        VALUES (
            CURRENT_USER,
            'UPDATE',
            TG_TABLE_NAME,
    ...
    ELSIF TG_OP = 'DELETE' THEN
        INSERT INTO "AuditLogs" ("user_id", "action", "entity_type", "entity_id", "old_values")
        VALUES (
            CURRENT_USER,
            'DELETE',
            TG_TABLE_NAME,
    ...
    ELSIF TG_OP = 'INSERT' THEN
        INSERT INTO "AuditLogs" ("user_id", "action", "entity_type", "entity_id", "new_values")
        VALUES (
            CURRENT_USER,
            'INSERT',
            TG_TABLE_NAME,
...
CREATE TRIGGER dog_walkers_audit
AFTER INSERT OR UPDATE OR DELETE ON "DogWalkers"
FOR EACH ROW EXECUTE FUNCTION log_audit_changes();
...
-- Create function to send notifications for order status changes
